...
export const downloadTodos = () => async (dispatch, getState) => {
    const {userLogin: {user}} = getState()
    ...
    try{
        ...
        console.log(err)
        dispatch({
            type: DOWNLOAD_TODO_FAIL,
            err: err.message
        })

        dispatch({
            type: CREATE_MESSAGE,
            payload: {message:"Download failed!", type: "error"}
        })
    }
}
